      ...
      <Container>
        {tasks.map(({ _id, title, description, finDate, estado }) => (
          <Card key={_id} className="mb-3">
            <Card.Body>
              <Card.Title className="mx-5">{title}</Card.Title>
              <Card.Text>{description}</Card.Text>
              <Card.Text>fecha de vencimiento:<br />{new Date(finDate).toLocaleString()}</Card.Text>
              <Card.Text>{estado}</Card.Text>
            </Card.Body>
            <Card.Body className="d-flex align-items-center">
              <Button onClick={() => completedTask({ _id, title, description, finDate, estado })}>  Marcar como Completada</Button>
              <ButtonGroup className="ms-auto">
                <Button className="mx-5" onClick={() => handleShowET({_id, title, description, finDate, estado})}> Editar</Button>
                <Button onClick={() => deleteTask(_id)}> Eliminar </Button>
              </ButtonGroup>
            </Card.Body>
          </Card>
        ))}
